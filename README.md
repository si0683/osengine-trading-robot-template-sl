...
| Лимитный ордер на вход (BuyAtLimit / SellAtLimit) | ✅ Готово |
| Рыночный ордер на вход (BuyAtMarket / SellAtMarket) | ✅ Готово |
...
| Пересчёт стопа от реального EntryPrice (Market-ордер) | ✅ Готово |
...
| `Order Type` | string | `Limit` | Тип ордера на вход: `Limit` или `Market` |
...
> **Параметр `Order Type`:** `Limit` — лимитный ордер, цена входа известна заранее. `Market` — рыночный ордер, стоп в % пересчитывается от реального `EntryPrice` в `OnPositionOpeningSucces`, чтобы стоп в деньгах оставался равным `RiskMoney` независимо от проскальзывания.
...
// Ключ словаря — pos.Number (всегда доступен сразу)
...
// Стоп хранится как % — OnPositionOpeningSucces пересчитает от реального EntryPrice
_stopByPosNumber[pos.Number] = new StopData
{
    StopPrice   = 0m,
    StopPct     = stopPct,
    ProfitPrice = _curProfitPercent, // тоже в % для пересчёта
    IsMarket    = true
};
```
...
- **Синхронный поток** — `CandleFinishedEvent`, где выставляется ордер (`BuyAtLimit` / `BuyAtMarket`)
...
### Решение — `ConcurrentDictionary<int, StopData>`

Шаблон хранит данные стопа в `ConcurrentDictionary`, где **ключ — `pos.Number`** (уникальный номер позиции, доступен сразу после вызова `BuyAtLimit`/`BuyAtMarket`).

> **Почему `pos.Number`, а не `order.NumberUser`?**
> При рыночном ордере `OpenOrders` заполняется асинхронно внутри `IcebergMaker` — к моменту возврата из `BuyAtMarket` список может быть ещё пустым. `pos.Number` всегда доступен сразу.

`StopData` содержит четыре поля:

| Поле | Описание |
|---|---|
| `StopPrice` | Абсолютная цена стопа (для лимитного ордера) |
| `StopPct` | % стопа от entry (для рыночного ордера) |
| `ProfitPrice` | Цена тейка или % (0 = не выставлять) |
| `IsMarket` | `true` = рыночный ордер, нужен пересчёт от `EntryPrice` |

```
Синхронно (CandleFinishedEvent)             Асинхронно (PositionOpeningSuccesEvent)
─────────────────────────────────           ─────────────────────────────────────────
1. Рассчитать stopPrice / stopPct           4. Биржа подтвердила исполнение
2. BuyAtLimit / BuyAtMarket → pos           5. OnPositionOpeningSucces(pos) вызван
3. Записать в словарь:                      6. TryRemove(pos.Number) — удаляет ключ
   _stopByPosNumber[pos.Number] =           7. IsMarket=false: CloseAtStop(stopPrice)
     new StopData { ... }                      IsMarket=true:  пересчёт от EntryPrice
                                            8. CloseAtProfit(profitPrice)
```

### Пересчёт стопа для Market-ордера
...
`OnPositionOpeningSucces` проверяет `pos.StopOrderRedLine > 0` — если стоп уже выставлен, повторная установка игнорируется:
...
Если ордер отклонён биржей, `PositionOpeningSuccesEvent` не вызывается. Чтобы запись не осталась навсегда, предусмотрен `OnPositionOpeningFail`:

```csharp
private void OnPositionOpeningFail(Position pos)
{
    if (_stopByPosNumber.TryRemove(pos.Number, out _))
        SendNewLogMessage($"[STOP] Стоп удалён из словаря (OpeningFail) pos#{pos.Number}", LogMessageType.System);
}
```
...
`CloseAtTrailingStop` двигает стоп только в прибыльную сторону. Вызывайте каждый бар — движок сам решит, нужно ли обновлять.

### Итог

| Подход | Гонка состояний | Несколько позиций | Утечка при неудаче | Market-ордер |
|---|---|---|---|---|
| Поле класса `_stopPrice` | ❌ Есть | ❌ Стопы перемешиваются | — | ❌ |
| `Dictionary<int, decimal>` | ⚠️ Нужен `lock` | ✅ | ❌ Без `OnOpeningFail` | ❌ |
| `ConcurrentDictionary<int, StopData>` + `TryRemove` + `OnOpeningFail` | ✅ Нет | ✅ | ✅ | ✅ |
...
В `LogicOpenPosition()` раскомментируйте и адаптируйте заготовку. Пример для лонга через лимитный ордер:
